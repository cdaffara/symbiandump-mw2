...
CPerformance424Client::CPerformance424Client(CPerformanceStepBase& aStepBase) 
: CActive(EPriorityStandard), iStepBase(aStepBase) 
	{
	CActiveScheduler::Add(this);
	}

CPerformance424Client* CPerformance424Client::NewL(CPerformanceStepBase& aStepBase)
	{
	CPerformance424Client* self = NewLC(aStepBase);
	CleanupStack::Pop(self);
	return self;
	}

CPerformance424Client* CPerformance424Client::NewLC(CPerformanceStepBase& aStepBase)
	{
	CPerformance424Client* self = new(ELeave) CPerformance424Client(aStepBase);
	CleanupStack::PushL(self);
	self->ConstructL();
	return self;
	}
	
CPerformance424Client::~CPerformance424Client()
	{
	Cancel();
...
void CPerformance424Client::ConstructL()
	{
...
void CPerformance424Client::StartL()
	{
...
void CPerformance424Client::DoCancel()
	{	
	__ASSERT_DEBUG(iMsvOperation!=NULL, User::Invariant());
	iMsvOperation->Cancel();
	}

void CPerformance424Client::CompleteSelf()
	{
...
void CPerformance424Client::RunL()
	{
...
TInt CPerformance424Client::RunError(TInt aError)
	{
	iStepBase.TestComplete(aError);
	return KErrNone;
	}
	
void CPerformance424Client::HandleImapConnectionEvent(TImapConnectionEvent /*aConnectionState*/)
	{
