...
CPerformance426Client::CPerformance426Client(CPerformanceStepBase& aStepBase) 
: CActive(EPriorityStandard), iStepBase(aStepBase) 
	{
	CActiveScheduler::Add(this);
	}

CPerformance426Client* CPerformance426Client::NewL(CPerformanceStepBase& aStepBase)
	{
	CPerformance426Client* self = NewLC(aStepBase);
	CleanupStack::Pop(self);
	return self;
	}

CPerformance426Client* CPerformance426Client::NewLC(CPerformanceStepBase& aStepBase)
	{
	CPerformance426Client* self = new(ELeave) CPerformance426Client(aStepBase);
	CleanupStack::PushL(self);
	self->ConstructL();
	return self;
	}
	
CPerformance426Client::~CPerformance426Client()
	{
	Cancel();
...
void CPerformance426Client::StartL()
	{
...
void CPerformance426Client::DoCancel()
	{	
	__ASSERT_DEBUG(iMsvOperation!=NULL, User::Invariant());
	iMsvOperation->Cancel();
	}

void CPerformance426Client::CompleteSelf()
	{
...
void CPerformance426Client::RunL()
	{
...
TInt CPerformance426Client::RunError(TInt aError)
	{
	iStepBase.TestComplete(aError);
	return KErrNone;
	}
	
void CPerformance426Client::HandleImapConnectionEvent(TImapConnectionEvent /*aConnectionState*/)
	{
