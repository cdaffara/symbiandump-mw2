...
open(FILE,"msg-imap-performance0_0kb.ini");
@data = <FILE>;
close(FILE);
...
for($entry_value=0; $entry_value < @entry_array; $entry_value++)
{
	for($size_value=0; $size_value < @size_array; $size_value++)
	{
		...
		$filename = "M:\\performance-ini\\msg-imap-performance";
		@data1 = @data;
		foreach(@data1)
		{
			if($_ =~ /1KBody/)
 		...
 		$filename = $filename.$entry_array[$entry_value]."_".$size_array[$size_value]."kb.ini";
 		open(FILE,">>$filename");
 		print FILE @data1;
 		close(FILE);		
	}
