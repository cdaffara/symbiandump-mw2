...
CTestCase2* CTestCase2::NewL(CHTTPTestUtils& aTestUtils)
	{
	CTestCase2* self = new (ELeave) CTestCase2(aTestUtils);
...
CTestCase2::CTestCase2(CHTTPTestUtils& aTestUtils)
: CPipeliningTestCase(), iTestUtils(aTestUtils)
	{
	}

void CTestCase2::ConstructL()
	{
	}

CTestCase2::~CTestCase2()
	{
	}
...
TInt CTestCase2::TransactionCount(TInt aConnectionIndex) const
	{
	TInt transCount = 0;
...
TInt CTestCase2::TotalTransactionCount() const
	{
	return KConn1TransCount+KConn2TransCount;
	}

TInt CTestCase2::ConnectionCount() const
	{
...
const TDesC8& CTestCase2::GetRawRequest(TInt aConnectionIndex, TInt aTransIndex)
	{
...
const TDesC8& CTestCase2::GetRawResponse(TInt aConnectionIndex, TInt aTransIndex)
	{
	__ASSERT_ALWAYS(aTransIndex<TransactionCount(aConnectionIndex), User::Invariant());
